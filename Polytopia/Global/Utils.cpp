...
{
	sf::Vector2f dir = Utils::Normalize(b - a);
	float diff = Utils::Distance(a, b);

	sf::Vector2f v = a + dir *(diff * t);
	if (clamping)
	{
		v = Utils::Clamp(v, a, b);
	}
...
void Utils::FlipX(sf::Sprite& sprite, bool Flip)
{
	sf::Vector2f scale = sprite.getScale();
	scale.x = abs(scale.x) * Flip ? -1 : 1;
